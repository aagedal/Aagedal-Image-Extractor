...
enum ProcessingState: Equatable {
    ...
    case writingMetadata(progress: Double)
    ...
    var isProcessing: Bool {
        switch self {
        case .extracting, .converting, .writingMetadata, .runningOCR: true
        default: false
        }
    }
    ...
    var progressValue: Double {
        switch self {
        case .pending: 0
        case .extracting(let p): p * 0.4
        case .converting(let p): 0.4 + p * 0.25
        case .writingMetadata(let p): 0.65 + p * 0.15
